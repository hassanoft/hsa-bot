...
export default {
  ...
  async execute(ctx) {
    ...
    if (result.url) {
      await ctx.sock.sendMessage(ctx.chatId, { image: { url: result.url }, caption: `🎨 ${ctx.text}` }, { quoted: ctx.msg });
    } else {
      await ctx.sock.sendMessage(ctx.chatId, { image: result.buffer, caption: `🎨 ${ctx.text}` }, { quoted: ctx.msg });
    }
  },
};
