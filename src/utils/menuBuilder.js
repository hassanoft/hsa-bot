...
export function buildIdentityBlock(ctx) {
  const name = ctx.sock.user?.name || ctx.sock.user?.verifiedName || ctx.sock.user?.notify || 'H$Λ BOT';
  const pingMs = Math.max(0, Date.now() - Number(ctx.msg.messageTimestamp) * 1000);
...
/** Tente de récupérer la photo de profil du compte WhatsApp connecté. Ne lève jamais d'erreur. */
export async function getConnectedProfilePicture(sock) {
  try {
...
export async function sendMenu(ctx, categoryArg) {
  ...
  const photo = await getConnectedProfilePicture(ctx.sock);
  if (photo) {
    await ctx.sock.sendMessage(ctx.chatId, { image: photo, caption: text }, { quoted: ctx.msg });
  } else {
